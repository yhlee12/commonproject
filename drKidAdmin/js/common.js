...
var _SelectBoxOption = (function () {
    ...
    function settingData(SelectBoxer, boxId, divFontClass, disableBool = false) {
        //template
        ...
        if (disableBool) {
            _wraper.classList.add('disableColor');
        }
        ...
        for (let i = 0; i < SelectBoxer.length; i++) {
            ...
            _liData.innerHTML = `<span class="${divFontClass}" data-text="${SelectBoxer[i]['text']}" data-values="${SelectBoxer[i]['value']}">${SelectBoxer[i]['viewText']}</span>`;
            _selectUl.appendChild(_liData);
            //Disable 일경우 Binding 제거.
            if (!disableBool) {
                //Event Binding
    ...
    function TemplaterSetup(div, index) {
        ...
        let _disAbleBool;
        try {
            _divFontClass = div.dataset.fontClass;
            _disAbleBool = div.dataset.disableBool;
        }
        catch {
            _divFontClass = undefined;
            _disAbleBool = false;
        }
        let _boxData;
        let _boxId = 'SelectBox-' + index;
        
        let _disAbleBoolFlag = _disAbleBool == "Y" ? true : false;
        //Style 값 세팅
        try {
        ...
        try {
            for (let i = 0; i < div.options.length; i++) {
                ...
                if (div.options[i].dataset['optionInnerText'] != undefined) { _textData = div.options[i].dataset['optionInnerText']; }
                else { _textData = div.options[i]['text']; }
                let boxOptionData = {
        ...
        _SelectBoxOption.settingData(_boxData, _boxId, _divFontClass, _disAbleBoolFlag);
    }

    return {
        TemplaterSetup: TemplaterSetup,
        setinit: setinit,
        getter: getter,
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.querySelectorAll('.drkid-select').forEach((div, index) => {
        _SelectBoxOption.TemplaterSetup(div, index);
    });
    /*
     * Picker 사용법
