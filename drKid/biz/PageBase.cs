...
namespace drKid
{
    public class PageBase : System.Web.UI.Page
    {
        ...
        private void checkRefferCodeSet()
        {
            if (!String.IsNullOrEmpty(Request["RECOMMEND_CODE"]))
            {
                _refferCode = Request["RECOMMEND_CODE"];
            }

            if(!String.IsNullOrEmpty(Session[GloBalD._RefferSesstion] as string) && String.IsNullOrEmpty(_refferCode))
            {
                _refferCode = Session[GloBalD._RefferSesstion].ToString();
            }

            if (!String.IsNullOrEmpty(_refferCode))
            {
                Session[GloBalD._RefferSesstion] = _refferCode;
            }
        }
        ...
        public virtual void basePageInit()
        {
            ...
            if (!Page.User.Identity.IsAuthenticated)
            {
                if (_mobileChecker)
                {
                    if (Page.IsCallback)
                    {
                        Response.Redirect("~/Source/m/member/login/M_L_LOGIN_PAGE.aspx?ReturnUrl=" + referrer); //23-08-18허정현 로그인페이지 경로문제관련 수정
                    }
                    else
                    {
