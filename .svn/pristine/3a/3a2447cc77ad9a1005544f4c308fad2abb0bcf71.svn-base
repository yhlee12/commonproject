...
{
    public partial class C_PRODUCT_PURCHASE_LODING : PageBase
    {
        public string paramssb = "";
        public string _authResultString = "";
        System.Collections.Generic.Dictionary<string, string> _resultMap = new System.Collections.Generic.Dictionary<string, string>();
        ...
        private void StartINIStdReturn()
        {
            ...
            // 승인URL은 web.config 혹은 https://manual.inicis.com 참조
            {
                ...
                try
                {
                    ...
                    _authResultString = authResultString;

                    jsonStr = JsonConvert.SerializeObject(_authResultString);
                    Session["resultJsonData"] = jsonStr;
                    //Response.Write("api통신 시작<br/>");
                    //Response.Write(authResultString);
                    ...
                    if ("0000".Equals((resultMap.ContainsKey("resultCode") ? resultMap["resultCode"] : "null")) && secureSignature.Equals(resultMap["authSignature"]))	//결제보안 추가
                    {
                        ...
                        * 여기에 가맹점 내부 DB에 결제 결과를 반영하는 관련 프로그램 코드를 구현한다.  
                          ...
                          [중요!] 승인내용에 이상이 없음을 확인한 뒤 가맹점 DB에 해당건이 정상처리 되었음을 반영함
                                  처리중 에러 발생시 망취소를 한다.
                        ******************************************************************************/
                        resultMap.Add("merchantData", merchantData);

                        //foreach (var kvp in resultMap)
                        //{
                        //    Response.Write("Key: " + kvp.Key + ", Value: " + kvp.Value);
                        //    Response.Write("<br/>");
                        //}
                    ...
                    Response.Write("<tr><th class='td01'><p>가맹점 관리 데이터</p></th>");
                    Response.Write("<td class='td02'><p>" + (resultMap.ContainsKey("merchantData") ? resultMap["merchantData"] : "null") + "</p></td></tr>");
                    Response.Write("<tr><th class='line' colspan='2'><p></p></th></tr>");
                    ...
                    {//빌링결제
                        ...
                        Response.Write("<td class='td02 BillKey'><p>" + (resultMap.ContainsKey("CARD_BillKey") ? resultMap["CARD_BillKey"] : "null") + "</p></td></tr>");
